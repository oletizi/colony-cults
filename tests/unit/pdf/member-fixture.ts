...
export interface WriteMemberFixtureOptions {
   ...
   * used when multiple members share a group root (matching production: one
   * `COLONY_ARCHIVE_ROOT` for every source, never a per-member root).
   * When provided, `cleanup()` is a no-op -- the caller that owns the shared
   * root removes it.
   */
  archiveRoot?: string;
}
