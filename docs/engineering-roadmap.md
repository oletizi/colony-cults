...
Make `pdf:build` render editions by reading EXCLUSIVELY from our own normalized archive, dissolving the per-source-archive friction instead of adding archive-by-archive special cases. Today the PDF build reads the committed snapshot, generated by a Gallica-coupled loader that parses gallica.bnf.fr arks out of the catalog_url and fails loud on any other archive (archive.org, HathiTrust, museum, ...); the ark is used only to build Gallica IIIF image URLs, and page-range extracts break because absolute folios (fNNN) are mapped to extract-relative translations (pNNN). Since the acquisition adapters (Gallica/IA/museum) already resolve every source into one uniform archive shape — object_store master key + sha256, per-page OCR + translation, folio provenance — the fix is a PDF-scoped archive-direct edition reader that consumes that uniform provenance: images from object_store (B2) only (no ark/IIIF), ordering + folio-translation correspondence from the source's own folio sequence, an explicit "untranslatable" marker so marked pages render blank-EN while unmarked missing translations still fail loud, and reproducibility via a pinned archive commit recorded in the colophon. `pdf:build` requires the archive clone and no longer depends on the committed snapshot (browser snapshot path out of scope). Unblocks PB-P055, PB-P054 (Gallica page-range extract), PB-P002, and any future source from any archive.
