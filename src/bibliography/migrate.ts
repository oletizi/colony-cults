...
const CANONICAL_GALLICA = 'Gallica / BnF';
const CANONICAL_SLQ = 'State Library of Queensland';
const ACTIVE_STATUSES = new Set(['collecting', 'collected', 'archived']);

/** Free-text acquisition status (tracker) -> closed vocab. */
const TRACKER_STATUS = new Map<string, string>([
  ['wanted', 'wanted'],
...
const MIRROR_STATUS = new Map<string, string>([
  ['pending', 'to-collect'],
  ['in-progress', 'collecting'],
  ['in progress', 'collecting'],
  ['complete', 'collected'],
...
function mapStatus(table: Map<string, string>, raw: string, kind: string): string {
...
function requireCell(row: Record<string, string>, key: string, where: string): string {
...
function canonicalizeArchive(raw: string): string {
...
function splitArchives(raw: string): string[] {
...
function extractArk(url: string): string | undefined {
  const match = url.match(/ark:\/12148\/[^\s?#]+/);
  return match === null ? undefined : match[0];
}

/** Extract the SLQ record id + call number from free-text notes, if present. */
function extractSlqIds(notes: string | undefined): string | undefined {
  if (notes === undefined) {
    return undefined;
  }
...
function detectKind(typeColumn: string): Source['kind'] {
...
function safeSlug(sourceId: string): string | undefined {
  try {
