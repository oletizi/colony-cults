...
- **VI. Composition Over Inheritance** — PASS **with one recorded deviation**. Narrow per-seam policy interfaces derived at the composition root and injected; the corpus is **not** a service locator; registries stay orthogonal. **Deviation (T013, archive-layout seam only):** `sourceLayout(sourceId)` deferred-composes its `ArchiveLayoutPolicy` from committed config on first use and memoizes it, rather than receiving it as a parameter. Two constraints force this: (1) FR-017 fixes `sourceLayout` as **sourceId-only and synchronous** — it is reached via `ocr → resolveFetchedDir → sourceLayout(id)`, a chain with no policy parameter; (2) the T001 characterization gate, which is the evidence for FR-010 byte-identity, calls `sourceLayout()` on a fresh import with no setup, so an install-or-throw slot would fail it. `installArchiveLayoutPolicy(policy)` remains the strict-injection seam and preempts composition. **The same deviation extends to a second seam (T023, source-filename policy)** at the ~15 `loadAllSources` call sites whose chains carry no composition-root parameter — those call `committedSourceFilenamePolicy()` explicitly, with `installSourceFilenamePolicy` as the strict-injection seam. In both cases the deferred call is written at the call site rather than hidden in a parameter default, so the choice is visible in review. Not a regression — the retired `SOURCE_LAYOUTS` map plus its mutable runtime overlay were already process-global; the corpus-specific data now comes from config rather than a literal. Cost acknowledged: a hidden filesystem dependency behind a signature that does not advertise it. Reversible by re-pointing the gate.
