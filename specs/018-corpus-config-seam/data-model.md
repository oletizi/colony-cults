...
**Browser defaults vs. their corpus** (config-only, so these run at startup too): a profile's `corpus` **is the corpus whose FILE it is** — the `<stem>` of `<stem>.browser.yml`, since `loadBrowserProfile` keys on the filename while `corpus:` only claims ownership (`profile-corpus-filename-mismatch`); and every `defaultSources` id conforms to **at least one of** that corpus's `sourceIds` policies (`profile-default-source-nonconforming`). Both were previously unchecked: `defaultSources` stopped at the loader's "array of non-empty strings" type check, so a profile could hand one corpus another corpus's ids. The same conformance check is applied at derivation time by `deriveBrowserProfile`, to the `CORPUS_SOURCES` override as well as to the file — an operator override of *which* ids to build is not an override of *what an id means*.
**Existing-data**: every existing Source ID globally unique; every Source under a Corpus conforms to **at least one of** its ID policies (FR-002b — this is what makes `PB-S001`/`PB-S002` valid without a grandfathering carve-out); the next allocated ID, drawn from the **allocatable** policy, cannot collide with any existing ID or another Corpus's namespace; every profile `defaultSources` id names a real SSOT record (`profile-unknown-default-source` — the one browser-defaults rule that needs the identity index, hence full-sweep only).
