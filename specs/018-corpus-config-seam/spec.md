...
The apparatus was built for a single research corpus (Port Breton). A coupling assessment found it is already ~80% corpus-neutral, with the real single-corpus coupling in **five localized spots**: the archive `SOURCE_LAYOUTS` map, the bibliography `PORT_BRETON_CASE_ID` constant, the `PB-P` source-ID allocator, the browser default source list, and — found during implementation — the `SOURCE_FILE_PATTERN` filename filter in `src/bibliography/load.ts` (FR-018). This feature introduces a **composition-time configuration seam** so the same shared core runs varied subjects: corpus identity/policy becomes validated **data manifests**, selected explicitly and injected as **narrow per-seam policies**. Port Breton becomes corpus-instance #1 as a **behavior-preserving extraction** — no data changes, no rewrite, one repository. Domain generalization (pluggable discovery + date normalizer) is **epic spec 2**, out of scope here.
...
- **FR-018 (source-filename policy — the fifth seam)**: `src/bibliography/load.ts` hardcodes `SOURCE_FILE_PATTERN = /^PB-[A-Z]?\d{3}\.yml$/`, which `loadAllSources` applies to every filename. This is a **fifth corpus-specific constant**, discovered during implementation (T005 and T007 both hit it independently). Left in place it **silently** hides a second corpus's Sources — a `SYN-001.yml` simply does not match, so `loadAllSources` returns zero Sources with no error — which would make SC-003/FR-011 either fail or, worse, pass vacuously against an empty list. It MUST therefore become an injected **`SourceFilenamePolicy`**, derived at the composition root from the corpus's `sourceIds` policies (a file is a Source file when its basename matches ANY of that corpus's `prefix` + `padWidth` shapes). The policy MUST be a **required** parameter: it must NOT default to the legacy pattern, because a default is exactly the silent fallback (Principle V) that hides the bug. Every `loadAllSources` call site threads the policy from the composition root, **except** the ~15 sites on chains with no composition-root parameter available (`sourceKind` via the fetch-source guardrail; `runAcquire`/`runReconcile`/`runPromote`, which take plain input records; the PDF batch builder, which runs from a bin that never composes a corpus). Those call an explicit `committedSourceFilenamePolicy()` — deferred composition over committed manifests, memoized, preemptable by `installSourceFilenamePolicy` — the same pattern recorded for T013's archive-layout seam. Called explicitly so the choice is visible at the call site rather than hidden in a default.
- **FR-019 (source-ID grammar is corpus-neutral — the sixth constant)**: `loadSourceFile` additionally hardcoded `SOURCE_ID_PATTERN = /^PB-[A-Z]?\d{3}$/`, a **sixth** corpus-specific constant the original assessment missed. It sits one layer below FR-018: even once `loadAllSources` enumerates a `SYN-001.yml`, that rule would reject it a line later, so INV-16 is unsatisfiable without addressing it. With 184 `loadSourceFile` call sites, injecting a policy there is not viable. The rule is therefore **deliberately loosened to a corpus-neutral shape grammar** `^[A-Z][A-Z0-9]*(?:-[A-Z0-9]+)*\d+$` — it now asserts only that an ID is *well-shaped*. **Which namespaces are legitimate is answered solely by the injected `SourceFilenamePolicy` at enumeration time and by existing-data validation (FR-002a/002b) at validate time.** This is a real reduction in what `loadSourceFile` alone rejects, accepted because the authority moved rather than disappeared.
...
- **SC-004**: None of the **five** corpus-specific constants remain as literals in core modules; each seam reads an injected policy.
