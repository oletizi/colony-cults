...
| **IV. Respect Copyright (Fail Closed)** | **Central.** Affirmative, controlled `Source.rights` required; `likely`/absent/non-distributable → refused, nothing uploaded/recorded (FR-002, SC-003). Every published variant that carries machine translation (both in-scope variants — `english-only` and `parallel`) carries a `machineAssist` label (translations MUST be labeled machine-assisted). |
