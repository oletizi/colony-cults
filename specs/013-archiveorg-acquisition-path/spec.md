...
- **FR-007**: `acquire` MUST fetch the chosen PDF to a local staging area and record its fixity (byte length, sha256) + Internet Archive file metadata; NO bytes reach the shared object store before the quality gate approves.
...
- **FR-009**: The master source MUST be selected from measured evidence — compare the PDF's extracted-image fidelity (dimensions/DPI/compression) against `scandata.xml`'s recorded page dimensions; explode the PDF when equivalent; fetch the `_jp2.zip` and use it only when the PDF is materially degraded.
- **FR-010**: Page-masters MUST be produced under a strict **page-to-leaf invariant** — per approved logical page, extract losslessly (`pdfimages`) only when the page contains exactly one suitable page image, else rasterise that page at a recorded DPI (`pdftoppm`); the produced count MUST be verified against the approved leaf range (fail loud on mismatch); each master MUST record its per-page method provenance (`method`, `sourcePdfObject` or `resolutionDpi`, `leaf`, `logicalPage`).
...
- **FR-012**: The repository-supplied PDF MUST be preserved as a `repository-source` `AcquiredAsset` alongside the `page-master` assets; downstream corpus tools consume only the page-master assets.
...
- **AcquiredAsset roles**: `repository-source` (the preserved PDF) and `page-master` (the per-page images) — a `role`/`sequence` distinction (whether a new field is needed vs `mediaType` is a plan-time model decision).
...
- Reuses the shipped `RepositoryAdapter` + registry (`src/repository/adapter.ts`), the New Italy Museum adapter as prior art (spec 011), the metadata-snapshot store (`src/sourcegroup/snapshot.ts`), the `bib` pipeline verbs, the project PDF machinery (`src/pdf/`), and poppler (`pdfimages`/`pdftoppm`/`pdfinfo`) — all present.
...
- **Five items are deferred to `/speckit-clarify` + `/speckit-plan` for scoping** (from the design record's open questions): (1) the concrete fidelity-comparison thresholds that decide "PDF demonstrably degraded → fetch JP2"; (2) the precise per-page test for "exactly one suitable page image" and the rasterisation DPI; (3) the staging location + cleanup lifecycle; (4) confirmation that the automated `advancedsearch` `DiscoveryMechanism` stays out of scope for v1; (5) whether `AcquiredAsset` needs a new `role`/`sequence` field or the distinction is carried by `mediaType`.
