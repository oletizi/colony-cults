...
- [ ] T004 [P] [tier:fast] Add `'ocr-text'` to the `AcquiredAsset` role union in src/model/acquired-asset.ts.
...
- [ ] T006 [tier:balanced] Implement the mechanical article parse in src/repository/papers-past/parse.ts (pure, HTML string → { articleId, title (h3), imageLocators sequenced by `area`, ocrText (`#text-tab`), newspaper/date/page, rightsRaw }); fail-loud on a missing article id or zero image locators. Reuse node-html-parser and the selectors confirmed in src/sourcequery/sources/papers-past-article.ts.
- [ ] T007 [P] [tier:balanced] Unit test src/repository/papers-past/parse in tests/unit/repository/papers-past/parse.test.ts against the de-rays fixture: article id `HNS18840103.2.19.3`, title contains "CONVICTION OF MARQUIS DE RAYS", 3 sequenced imageserver locators, OCR contains "found guilty and sentenced to four years", rightsRaw contains "No known copyright"; parse of a non-article page throws.

## Phase 3: User Story 1 — Acquire one public-domain article end-to-end (P1) 🎯 MVP

**Goal**: `bib acquire` mirrors a PD Papers Past article's page-image facsimile + OCR into archive + B2, idempotent + dry-run-safe.
**Independent test**: drive `acquire` on a public-domain record with fakes → page-master + ocr-text assets put under `archive/papers-past/<id>/`, idempotent re-run, dry-run writes nothing.
...
- [ ] T016 [P] [US2] [tier:balanced] Implement `PapersPastAdapter.collectRightsEvidence` in src/repository/papers-past/adapter.ts: map the verbatim "No known copyright (New Zealand)" rightsRaw + `jurisdiction: 'NZ'` + the grounded article date; NO `rightsStatus`.
...
- [ ] T020 [P] [US3] [tier:fast] Add `papers-past` to the `bib inventory` repository allowlist / enumeration in src/cli/bib-sourcegroup.ts (parity with gallica/museum/internet-archive).
...
- [ ] T023 [P] [tier:fast] Add a one-time live-acquire smoke note to quickstart.md (Scenario 6) confirming the held facsimile + OCR + provenance after a real `bib acquire`.
...
- Foundational: T002, T003, T004 in parallel (distinct files); T007 parallel after T006.
