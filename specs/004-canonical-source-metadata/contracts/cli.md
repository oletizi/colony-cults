...
New verb group beside the existing `bib` verbs (`src/cli/bibliography.ts`, wired through `src/cli/dispatch.ts`). Text in / text out; errors → stderr; non-zero exit on validation findings (matches the repo's fail-loud posture).
