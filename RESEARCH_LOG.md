...
- **Spec 013 (Internet Archive acquisition path)** — built the third first-class RepositoryAdapter (`internet-archive`, `ia-item` copies), the shipped seam's implementation for archive.org text items: `resolve`/`collectRightsEvidence`/`acquire`, registered in the dispatch registry and wired into `bib acquire`/`bib inventory`. Reaches the document frugally (one PDF download to staging), a fail-closed quality gate before any B2 write, evidence-selected master (dimension-ratio probe: explode the PDF, or fetch the full-resolution image set `_jp2.zip`/`_tif.zip` when the PDF is materially degraded), strict page-to-leaf extraction with per-page method provenance, the source PDF preserved as a `repository-source` asset, and a two-phase flag-driven quality gate (`--dry-run` examine → `--approved-range` commit) that re-reads the staged master with zero re-download (Principle XII). Fully unit-tested against the real de Groote metadata/scandata fixtures with injected fakes (no network/B2/poppler). **First live acquisition is the de Groote 1880 book `nouvellefrancec00groogoog` (SRCH-0013 target, TASK-32) — pending as the operator-acceptance run (spec 013 tasks.md T055), which measures and confirms the fidelity threshold against the real item.**
